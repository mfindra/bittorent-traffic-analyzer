...
def extract_bootstrap_nodes(csv_file):
    ...
    with open(csv_file, newline='') as csvfile:
        ...
        for row in csv_reader:                                 
            if row and 'get_peers' in str(row) and str(row[6]).endswith("y,q") and str(row[6]).startswith("a,bs,id"):
...
def extract_peers(csv_file):
    ...
    with open(csv_file, newline='') as csvfile:
        csv_reader = csv.reader(csvfile, delimiter=';')
        for row in csv_reader:
            if row and 'nodes' in str(row) and str(row[6]).endswith("y,r"):
                ip_field = row[4]
                port_field = row[5]
                ids_field = row[9]
                ...
                for i in range(len(n_flags)):
                    if n_flags[i] == '1':
                        peers_nodes.add((ips[i], int(ports[i]), ids[i]))
    ...
    return sorted(list(peers_nodes), key=lambda x: x[1])
...
def process_pcap(pcap_file, mode):
    with tempfile.NamedTemporaryFile(mode='w+t', delete=False) as temp_csv:
        tshark_command = [
            ...
            '-e', 'frame.time_relative', '-e', 'ip.src', '-e', 'ip.dst', '-e', 'udp.dstport', '-e', 'bt-dht.ip', '-e', 'bt-dht.port',
